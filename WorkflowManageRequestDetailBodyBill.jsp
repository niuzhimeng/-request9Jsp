				...
				/*if(linkurl.indexOf("HrmSubCompanyDsp.jsp?")>-1){
		    		...
		    		linkurl+="?f_weaver_belongto_userid="+userid+"&f_weaver_belongto_usertype="+usertype;
                                   ...
                                   <%
	                                              ...
	                                              %>
	                                                 ...
	                                                 <% if(showtype == 2){%>
	                                                     <input type='checkbox' 
	                                                       ...
	                                                       value="<%=tmpselectvalue %>" <% if(("," + defieldvalue + ",").indexOf("," + tmpselectvalue + ",") >=0){ %> checked <%} %>
                                                           viewtype='<%=isdemand%>'
	                                                       onClick="
			                                                       selectItemCheckChange(this,'<%=defieldid + "_" + derecorderindex%>');
	                                                 ...
	                                                 <%} else{%>
	                                                     <input type='radio' 
	                                                       ...
	                                                       value="<%=tmpselectvalue %>" <% if(defieldvalue.equals(tmpselectvalue)){ %> checked <%} %>
	                                                       viewtype='<%=isdemand%>'
                                                           onClick="
		                                                           selectItemCheckChange(this,'<%=defieldid + "_" + derecorderindex%>');
		                                                           checkinput2('field<%=defieldid%>_<%=derecorderindex%>','field<%=defieldid%>_<%=derecorderindex%>span',this.getAttribute('viewtype'));
                                        ...
                                        if( isdeedit.equals("0") || !isdeview.equals("1") || colcount1==0 || !editdetailbodyflag || showtype == 2 || showtype == 3) {
                                           %>
                                           <input type= hidden id="field<%=defieldid%>_<%=derecorderindex%>" name="field<%=defieldid%>_<%=derecorderindex%>" 
                                           ...
                                           %>
		                                     viewtype='<%=isdemand%>' temptitle='<%=defieldlable%>'
                                           <% 
                                        }
                                           %>
                                           <%
                                           if(showtype == 3){
	                                           if(seldefieldsdemanage.indexOf(defieldid)>=0){%>
	                                               onpropertychange="changeshowattr('<%=defieldid%>_1',this.value,<%=derecorderindex%>,<%=workflowid%>,<%=nodeid%>);
	                                               <%if (trrigerdetailStr.indexOf("field" + defieldid +"_"+derecorderindex) >= 0) {%>
	                                                   datainputd('field<%=defieldid%>_<%=derecorderindex%>');
	                                               <%}%>
	                                               <%=onchangeAddStr%>"
	                                           <%}else{%>
	                                               onpropertychange="
                            	...
                            	while (selrs.next()) {
                                    ...
                                    if(showtype == 1){
                                        ...
                                        if(showtype == 2){
                                            ...
                                            fieldhtml += " onclick=";
                                            fieldhtml += "selectItemCheckChange(this,'" + defieldid + "_\"+rowindex+\"');checkinput3(field" + defieldid + "_\"+rowindex+\",field" + defieldid + "_\"+rowindex+\"span,this.viewtype);changeshowattr('"+defieldid+"_1',this.value,\"+rowindex+\","+workflowid+","+nodeid+");"+trrigerdetailStr+onchangeAddStr+" ";
                                            //if(seldefieldsdemanage.indexOf(defieldid)>=0){
