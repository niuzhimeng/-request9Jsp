...
<%
	...
	String nodename = "";
	if(!"".equals(ehnextnodeid)){
		RecordSet rs = new RecordSet();
		rs.executeQuery("select nodename from workflow_nodebase where id=?", ehnextnodeid);
		if(rs.next())
...
<wea:group context='<%=SystemEnv.getHtmlLabelName(124954, user.getLanguage()) %>'>
	<wea:item><%=SystemEnv.getHtmlLabelName(383029, user.getLanguage()) %></wea:item>
	<wea:item>
		<span><%=nodename %></span>
	</wea:item>
