...
if ("".equals(tempcurrentnodetype) && requestid == -1) {
 ...
 String fieldid_temp = fieldid.substring(5);
 String[] fieldidarray = fieldid_temp.split("_");
 if( fieldidarray.length > 1)
	 fieldid_temp = fieldidarray[0];
 
 FileUploadDTO fudto = FieldFileUploadUtil.getFileUploadInfo(fieldid_temp,workflowid+"",requestid+"");
...
%>
    <tr>
            ...
            if(uploadType == 0)//固定目录
  				maxUploadImageSize = fudto.getMaxUploadFileSize();
            else{//选择目录
            	String doccategory = FieldFileUploadUtil.getSelectCategory(workflowid,temp_SelectedCateLog,selectfieldvalue);
            	String secId_temp = "";
            	if(!"".equals(doccategory)){
        			String[] categoryArray = doccategory.split(",");
        			if(categoryArray.length == 3){
        				mainId = categoryArray[0];
        				subId = categoryArray[1];
        				secId = categoryArray[2];
        			}else{
        				mainId = "";
        				subId = "";
        				secId = doccategory;
          ...
          System.out.println("selectfieldvalue="+selectfieldvalue);
          if(uploadType == 1 && "".equals(selectfieldvalue)){
        	  canupload=false;
        	  String fieldnametemp = RequestBaseUtil.getFieldLabel(temp_SelectedCateLog,workflowid,user.getLanguage());
        	  errmsg = SystemEnv.getHtmlLabelName(125389,user.getLanguage()) + "“"+fieldnametemp+"”！";
          }else{
	          if(("".equals(mainId) && "".equals(subId) && "".equals(secId)) || !isCanuse){
	          		canupload=false;
	          		errmsg = SystemEnv.getHtmlLabelNames("125389,92,15808",user.getLanguage()) + "！";
	           }
          }
          if(!canupload){
        	  %>
              ...
              <td colspan="2">
                  <div>
                      ...
                      &nbsp;&nbsp;
								...
								</span><span id="uploadspan_<%=fieldid %>">(<%=SystemEnv.getHtmlLabelName(18976,user.getLanguage())%><%=maxUploadImageSize%><%=SystemEnv.getHtmlLabelName(18977,user.getLanguage())%>)</span>
