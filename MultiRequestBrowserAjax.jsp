...
//				String titles=MailAndMessage.getTitle(Util.getIntValue(request_id,-1),Util.getIntValue(workflowid,-1),formid,user.getLanguage(),isbill);
		...
		if (prjidAry.length > 0) {
			...
			if ("oracle".equals(RecordSet.getDBType())) {
				...
				for (int i = 0; i < prjidAry.length; i++) {
					...
					sqlwhere.append("(concat(',' , a.prjids , ',') LIKE '%,"
							+ prjidAry[i] + ",%')");
				}
			} else {
				for (int i = 0; i < prjidAry.length; i++) {
		...
		if (crmidAry.length > 0) {
			...
			if ("oracle".equals(RecordSet.getDBType())) {
				...
				for (int i = 0; i < crmidAry.length; i++) {
					...
					sqlwhere.append("(concat(',' , a.crmids , ',') LIKE '%,"
							+ crmidAry[i] + ",%')");
				}
			} else {
				for (int i = 0; i < crmidAry.length; i++) {
	...
	if (RecordSet.getDBType().equals("oracle")) {
		...
		if(!"flowrpt".equals(isfrom)){
				sqlwhere.append(" and (ifnull(a.currentstatus,-1) = -1 or (ifnull(a.currentstatus,-1)=0 and a.creater="
						+ user.getUID() + ")) ");
		}
	} else {
		if(!"flowrpt".equals(isfrom)){
   ...
   // String sql_counts="select count(a.requestid) RecordSetCounts from workflow_requestbase a"
	...
	if (RecordSet.getDBType().equals("oracle")) {
		...
		sqlstr = "SELECT a.requestid,a.requestname,a.creater,a.createdate,a.createtime,a.creatertype FROM workflow_requestbase a, workflow_currentoperator b, workflow_base c where 1=1 " + sqlwhere.toString ().replaceAll("receivedate", "createdate") +sqlend + " limit " + (pagenum - 1) * perpage + " , " + pagenum * perpage;
	}else{
