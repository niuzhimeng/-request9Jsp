...
function windowOnload(){
    ...
    try{
	    jQuery("table[id^='formremark_']").each(function(){
			var color = jQuery(this).closest("td").css("border-top-color");
			if(color == 'rgb(0, 0, 0)'){
...
{ jQuery($GetEle("flowbody")).attr("onbeforeunload", "");
$G("fileDownload").src="/weaver/weaver.file.FileDownload?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&fileid="+files+"&download=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>";
}
	            ...
	            <%
	                if(RecordSet.getDBType().equals("mysql"))
	                    RecordSet.executeSql("select * from("+
	                            ...
	                            "workflow_function_manage manage left join workflow_freeright rights on manage.operatortype=rights.nodeid)"+
	                            " union (select rights.nodeid,manage.retract,manage.pigeonhole,rights.isroutedit,rights.istableedit from "+
                                "workflow_function_manage manage right join workflow_freeright rights on manage.operatortype=rights.nodeid)"+
...
function openSignPrint() {
	var redirectUrl = "PrintRequestForward.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&isprint=1&fromFlowDoc=1";
...
function doSubmitBack(obj){
	...
	}else if(isFree == "1" && freewftype == "1" && nodetype == "0" && isremark == "0" && takisremark == "0"){
...
jQuery(document).ready(function(){
	...
	eh_dialog.URL = "/workflow/request/requestChooseOperator.jsp?ehnextnodeid="+ehnextnodeid;
