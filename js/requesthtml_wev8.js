...
function toFix(value, decimalnum) {
	var result;
	if (!isFinite(value) || isNaN(value)) {
		...
		if(value_str.indexOf(".") > -1){
			var intParam = value_str.substring(0, value_str.indexOf("."));
			if(intParam == "")	intParam = "0";
			var pointParam = "0"+value_str.substring(value_str.indexOf("."));
			if(pointParam.length > 8){	//数值运算结果精度误差问题，先变6位
				pointParam = Number(pointParam).toFixed(6);
			}
			var addStr = "5";
			...
			addStr = "0." + addStr;
			//四舍五入位+5做截取模拟
			pointParam = (Number(pointParam) + Number(addStr)).toString().substring(0, decimalnum+2);
			if(value > 0)
				result = Number(intParam) + Number(pointParam);
			else
				result = Number(intParam) - Number(pointParam);
			result = result.toFixed(decimalnum);
		}else{
			result = value.toFixed(decimalnum);
		}
	}
	return result;	
}
