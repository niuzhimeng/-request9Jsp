...
int dycsfieldvalue=0;
RecordSet.executeSql("select count(*) as printcount from workflow_viewlog where requestid='"+requestid+"'");
 if(RecordSet.next()){
	 dycsfieldvalue=Util.getIntValue(RecordSet.getString("printcount"),0);
 }
...
function setmantable(){
    ...
    try{
	    ...
	    //打印次数
	    var temprow9=wcell.GetCellUserStringValueRow("main_dycs");
    	var tempcol9=wcell.GetCellUserStringValueCol("main_dycs");
    	
    	if(temprow9>0){
	   	  	isProtect=wcell.IsCellProtect(temprow9,tempcol9);
		    if(isProtect){
			    wcell.SetCellProtect(temprow9,tempcol9,temprow9,tempcol9,false);
		    }
           wcell.SetCellVal(temprow9,tempcol9,getChangeField('<%=dycsfieldvalue%>'));
           imgshoworhide(temprow9,tempcol9);
        }
...
<%
 ...
 int isprint=Util.getIntValue(Util.null2String(request.getParameter("isprint")));
 int printmodeid=Util.getIntValue(Util.null2String(request.getParameter("printmodeid")));
 if(isprint==1){
     FieldInfo.setIsprint(1);
     if(printmodeid>0){
         FieldInfo.setPrintModeId(printmodeid);
     }
 }
for(int i=0;i<NodeList.size();i++){
    String nodestr=(String)NodeList.get(i);
