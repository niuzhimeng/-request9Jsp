...
if (isoracle) {
  sqlFormprop = "select objtype,isdetail,objid,fieldid,attribute2,attribute1,defvalue,trunc(Round((trunc(ptx)-30)/30.0,0)*30) as ptx1,trunc(Round((trunc(pty)-30)/30.0,0)*30) as pty1,trunc(Round(trunc(width)/30.0,0)*30) as width1,trunc(Round(trunc(height)/30.0,0)*30) as height1 from workflow_formprop where formid="+formid+"order by trunc(pty),trunc(ptx)";
	...
	sqlFormprop = "select objtype,isdetail,objid,fieldid,attribute2,attribute1,defvalue,convert(Round((convert(ptx,SIGNED)-30)/30.0,0)*30,SIGNED) as ptx1,convert(Round((convert(pty,SIGNED)-30)/30.0,0)*30,SIGNED) as pty1,convert(Round(convert(width,SIGNED)/30.0,0)*30,SIGNED) as width1,convert(Round(convert(height,SIGNED)/30.0,0)*30,SIGNED) as height1 from workflow_formprop where formid="+formid+"order by convert(pty,SIGNED),convert(ptx,SIGNED)";
}else{
