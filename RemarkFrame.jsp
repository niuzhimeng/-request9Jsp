...
function transToUnHandItems() {
	...
	for (var i = 0; i < atitems.length; i++) {
		//if (atitems[i].handed === '0' && atitems[i].uid !== '<%=userid%>') {
		if (atitems[i].handed === '0') {
			if (i === (atitems.length - 1)) {
				transtounhanditemsids = transtounhanditemsids + atitems[i].uid;
...
%>
<%
	String tmpURL = "/systeminfo/BrowserMain.jsp?url=/hrm/resource/MutiResourceBrowser.jsp?resourceids=&f_weaver_belongto_userid=" +userid+"&f_weaver_belongto_usertype="+usertype;
	String dataURL = "/data.jsp?f_weaver_belongto_userid=" +userid+"&f_weaver_belongto_usertype="+usertype;
%>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("82578,33361",user.getLanguage())%>'>
		...
		<wea:item>
			...
			<brow:browser onpropertychange="wfbrowvaluechange(this, 5);" 
				...
				browserOnClick="" browserUrl="<%=tmpURL %>" hasInput="true" width="500px" isSingle="false"  hasBrowser = "true" isMustInput='2' completeUrl="<%=dataURL%>"  browserSpanValue='<%=resourcenames%>' hasAdd="true" addOnClick="showrescommongroup(this, 5)"> 
			</brow:browser>
...
<%}else if(3==forwardflag){%>
	<wea:group context='<%=SystemEnv.getHtmlLabelName(84510,user.getLanguage())%>'>
		...
		<wea:item>
			...
			<brow:browser  onpropertychange="wfbrowvaluechange(this, 5);"
				viewType="1" name="field5" browserValue='<%=resourceids%>' 
				browserOnClick="" browserUrl="<%=tmpURL %>" hasInput="true" width="500px" isSingle="true"  hasBrowser = "true" isMustInput='2' completeUrl="<%=dataURL%>"  browserSpanValue='<%=resourcenames%>' > 
				</brow:browser>
...
function getResource(tdname,inputename) {
	id = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/hrm/resource/MutiResourceBrowser.jsp?f_weaver_belongto_userid=" + <%=userid%>+"&f_weaver_belongto_usertype="+<%=usertype%>);
	if (id) {
...
%>
		if(ischeckok=="true"){
			if(check_form(frmmain,'field5'+'<%=needcheck%>')){
				...
				var parentWinForFormula = window.parent.parent.bodyiframe;
                var formulaMessage;
                if (forwardflag == 1 && typeof parentWinForFormula.runDoReviewFormula === "function") {
                    //转发时执行公式
                    formulaMessage = parentWinForFormula.runDoReviewFormula();
                } else if (forwardflag == 2 && typeof parentWinForFormula.runDoReview2Formula === "function") {
                    //意见征询时执行公式
                    formulaMessage = parentWinForFormula.runDoReview2Formula();
                } else if (forwardflag == 3 && typeof parentWinForFormula.runDoReview3Formula === "function") {
                    //转办时执行公式
                    formulaMessage = parentWinForFormula.runDoReview3Formula();
                }
                if (formulaMessage) {
                    alert(formulaMessage);
                    console.log("公式执行结果存在返回值控制流程流转：" + formulaMessage);
                    return;
                }				
<%
//保存签章数据
