...
<jsp:useBean id="wfManager" class="weaver.workflow.workflow.WFManager" scope="page"></jsp:useBean>

<%

int mywfid = Util.getIntValue(Util.null2String(request.getParameter("workflowid")));
String requestid = Util.null2String(request.getParameter("requestid"));
String workflowid = Util.null2String(request.getParameter("workflowid"));
String isMobileForm = Util.null2String(request.getParameter("isMobileForm"));
wfManager.setWfid(mywfid);
wfManager.getWfInfo();
String isFree = wfManager.getIsFree();
String freewftype = wfManager.getFreewftype();   //自由流程的模式，1：简易模式，2：高级模式
if("1".equals(isFree) && "1".equals(freewftype)){   //自由流程的建议模式
	request.getRequestDispatcher("/workflow/request/FreeNodeWin.jsp").forward(request, response);
	return ;
}
...
if("1".equals(isMobileForm)) {
    response.sendRedirect("/workflow/workflowDesign/m-readOnly.html?isFromWfForm=true&requestId="+requestid+"&workflowId="+workflowid+"&authStr="+Util.null2String(request.getParameter("authStr"))+"&authSignatureStr="+Util.null2String(request.getParameter("authSignatureStr")));
    return ;
} else if(isuseNewDesign == 1){
    response.sendRedirect("/workflow/workflowDesign/readOnly.html?isFromWfForm=true&requestId="+requestid+"&workflowId="+workflowid+"&authStr="+Util.null2String(request.getParameter("authStr"))+"&authSignatureStr="+Util.null2String(request.getParameter("authSignatureStr")));
    return ;
}
...
int nodexsize = 60;
int nodeysize = 40;
String nownodeids=WFLinkInfo.getNowNodeids(Util.getIntValue(requestid));
...
<TABLE cellpadding=1 cellspacing=1 Class=ChartCompany STYLE="table-layout : fixed;POSITION:absolute;Z-INDEX:100;FONT-SIZE:8pt;LETTER-SPACING:-1pt;TOP:<%=drawypos-nodeysize+top0 - 2%>;LEFT:<%=drawxpos-nodexsize+left0%>; height:<%=nodeysize*2 - 2%>!important;width:<%=nodexsize*2%>" LANGUAGE=javascript
    ...
    <%
      //modify by xhheng @20050106 for TD 1544
        } else if(curnodetype==1 && !requestid.equals("-1")){
    %>
