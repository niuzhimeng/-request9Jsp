...
<HTML>
<%
String isfromE9 = Util.null2String(request.getParameter("isfromE9"));
...
if(!prjids.equals("")&&!prjids.equals("0")){
	if(ishead==0){
		...
		if(RecordSet.getDBType().equals("oracle")){
			...
			sqlwhere += " and (concat(',' , t1.prjids , ',') LIKE '%,"+prjids+",%') ";
		}else{
...
function submitData() {
	var isfromE9 = <%="1".equals(isfromE9)%>;
    if(_xtable_CheckedRadioId()=="") {
	    ...
	    window.top.Dialog.confirm("<%=SystemEnv.getHtmlLabelName(24274,user.getLanguage())%>",function(){ 
	       if(isfromE9){
	       		dialog.close(_xtable_CheckedRadioId());
	       }else{
...
<input name=isfromE9 type=hidden value="<%=isfromE9%>">
