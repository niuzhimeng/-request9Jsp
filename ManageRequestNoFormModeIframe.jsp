...
boolean needcheckLock = LockUtil.isNeedCheckLock(requestid,workflowid,nodeid,isremark,takisremark,userid);
String delselfLock = (String)session.getAttribute(userid+"_"+requestid+"delselfLock");
session.removeAttribute(userid+"_"+requestid+"delselfLock");
boolean islock = false;

if(needcheckLock){
	islock = LockUtil.isLocked(requestid+"",nodeid+"").isLock();
	if(!"1".equals(delselfLock)){//校验流程是否被锁定
		if(islock){
			%>
			<script type="text/javascript">
				top.window.location.href = "/workflow/request/ViewRequest.jsp?requestid=<%=requestid%>";
			</script>
			<%
			return;
		}
...
RecordSet.executeSql("select ismode,showdes,isFormSignature,toexcel,isformprint,isformprintLog from workflow_flownode where workflowid="+workflowid+" and nodeid="+nodeid);
if(RecordSet.next()){
...
if("".equals(submitname)){
	if(isremark == 1 || isremark == 9||isremark == 7){
		...
		submitname = !"".equals(default_submitName_0) ? default_submitName_0 : SystemEnv.getHtmlLabelName(615,user.getLanguage());      // 创建节点或者转发, 为批注
...
if("".equals(subbackName)){
		if( isremark == 1 || isremark == 9||isremark == 7)	{
			...
			if((isremark==1 && ("1".equals(hasforback) && forsubbackCtrl == 2)) || (isremark==9 && ("1".equals(hasccback) && ccsubbackCtrl == 2))){
				subbackName += "（"+SystemEnv.getHtmlLabelName(21761,user.getLanguage())+"）";      // 创建节点或者转发, 为提交
			}
		}else if(nodetype.equals("0"))	{
...
if("".equals(subnobackName)){
	if( isremark == 1 || isremark == 9 ||isremark == 7)	{
		...
		if(subbackCtrl == 2) {
		subnobackName += "（"+SystemEnv.getHtmlLabelName(21762,user.getLanguage())+"）";      // 创建节点或者转发, 为提交
		}
    ...
    <table width="100%" id="t_help">
        <tr><td width="80%" align=left>
			<jsp:include page="wfSubmitErrorMsg.jsp">
       			...
       			<jsp:param name="islock" value="<%=islock %>" />
       			<jsp:param name="needcheckLock" value="<%=needcheckLock %>" />
...
if (window.addEventListener){
    ...
    window.addEventListener("unload", unloackRequest, false);
}else if (window.attachEvent){
    window.attachEvent("onload", windowOnload);
    window.attachEvent("onunload", unloackRequest);
}else{
    window.onload=windowOnload;
    window.onuload = unloackRequest;
}
...
function doSubmitBack(obj){
	var isFree = "<%=isFree%>";
	var freewftype = "<%=freewftype%>";
	var nodetype = "<%=nodetype%>";
	var isremark = "<%=isremark%>";
	var takisremark = "<%=takisremark%>";
...
function doFreeWorkflow(){
	...
	　　//dlg.URL = "/workflow/request/FreeNodeShow.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&workflowid=<%=workflowid%>";
	   ...
	   <%}else{%>
	     dlg.Width = 850;//定义长度
		 dlg.Height = 600;
	     dlg.URL = "/workflow/request/FreeNodeWin.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&requestid=<%=requestid%>&isdialog=1&isset=1&tt=<%=new Date().getTime()%>";
	    <%}%>
...
　　	var dialog = new window.top.Dialog();
    ...
    <%
     if(!freedis.equals("")&&!isornotFree.equals("")){
		String zjclNodeid=stnode.getIsFreeStart01Node(""+nodeid);
		if("1".equals(isFree) && "1".equals(freewftype)){   //自由流程，简易模式
			zjclNodeid = freeWorkflowSimple.getRejectToNodeId(requestid,nodeid,user);
		}
		//逐级退回
		if(freedis.equals("1")){
		   if(isrejectremind.equals("1")&&ischangrejectnode.equals("1")){%>
...
jQuery(document).ready(function(){
	...
	var ehnextnodeid = jQuery("input[name='ehnextnodeid']",window.parent.document).val();
	if(needChooseOperator != 'y')
		return;
	window.setTimeout(function(){
		var url = "/workflow/request/requestChooseOperator.jsp?ehnextnodeid="+ehnextnodeid;
...
<%if(needcheckLock){
	...
	RecordSet.next();
	int lockscan = Util.getIntValue(Util.null2String(RecordSet.getString("lockscan")),5);
	if(lockscan == 0)
		lockscan = 5;
%>
	jQuery(function(){
		...
		var intval = setInterval(function(){
			jQuery.ajax({
			    url: "/workflow/request/checkLockAjax.jsp?type=updatelock&requestid=<%=requestid%>&userid=<%=userid%>&nodeid=<%=nodeid%>&timestamp=<%=timestamp%>",
			    dataType: "text", 
				type:"post",
				async:true,
