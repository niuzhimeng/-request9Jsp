...
<%
    ...
    if(hasSession == false){
    	...
    	rs.execute("select remark,HandWrittenSign,remarklocation,workflowid from workflow_requestlog where logid="+logid);
    	if(rs.next()){
    		String remark_db = weaver.general.Util.null2String(rs.getString(1));
    		int workflowid = rs.getInt(4);
    		String isshowSrc = wFLinkInfo.getIsshowSrc(workflowid);
    		if(!"1".equals(isshowSrc)){
    			remark_db = wFLinkInfo.removeRemarkSource(remark_db);//删除意见来源
    		}
