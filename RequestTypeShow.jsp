...
String wfcrtSqlWhere = shareManager.getWfShareSqlWhere(BelongtoUser, "t1");
 sql="select distinct workflowtype from ShareInnerWfCreate t1,workflow_base t2 where t1.workflowid=t2.id   and t2.isvalid='1' and t2.islockworkflow = '0' and t1.usertype = " + usertype + " and " + wfcrtSqlWhere;
	...
	while(RecordSet.next()){
		...
		*/
		...
		if(haswfcreateperm){
	      ...
	      //QC210182  huangbh 代理问题（A代理B，B无该类别权限，无法查看A代理的流程）
	        ...
	        if(indx==-1){
				...
				//QC227132 zhudl 用户id之间互相包含,(2,3,24,这样的话不显示)代理人员身份数据显示不正确
									...
									if(fromAdvancedMenu!=1 && usertype==0&&!commonuse.equals("0") && usedtodo == 1){   //常用流程
                                        ...
                                        if(RecordSet.getDBType().equals("oracle")){
                                          if(offical.equals("1")){
											...
											if(offical.equals("1")){
                                              RecordSet.execute("select (case when workflow_base.activeversionid is not null then workflow_base.activeversionid else workflow_base.id end) as wfid,WorkflowUseCount.userid,sum(count) as count from WorkflowUseCount inner join workflow_base on workflow_base.id = WorkflowUseCount.wfid " +
