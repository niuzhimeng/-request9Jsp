...
//add by fanggsh for TD4528   20060621 end
       ...
       <%
            ...
            if("0".equals(isbill)){
                rs1.executeQuery("select childfieldid,type,fieldshowtypes from workflow_formdict where id= ?" ,fieldid);
            }else{
                rs1.executeQuery("select type,fieldshowtypes from workflow_billfield where id=?",fieldid);
            }
            if(rs1.next()){
                fieldshowtypes = Util.getIntValue(rs1.getString("fieldshowtypes"), 1);
            }
            //判断表示形式和排列方式
       ...
       <%
                    ...
                    <% if(fieldtype.equals("2")){%>
                        ...
                        <% if(("," + fieldvalue + ",").indexOf("," + tmpselectvalue + ",") >=0){ %> checked <%} %>
                        DISABLED >
                    <%} else{%>
                        <input type='radio' name="field<%=fieldid%>" 
                        <% if(fieldvalue.equals(tmpselectvalue)){ %> checked <%} %>
                        DISABLED >
                    <%} %>
                    <label ><%=tmpselectname%></label>
                    </div>
                 <%
            }
            }
           ...
           <%}%> 
              ...
              <td >
               <div style="float:left;height:40px; line-height:38px;width:70px;padding-left:10px;" class="fieldClassChange">
                ...
                <nobr>
	                  ...
	                  <%-- 
	                  <button style="color:#123885;border:0px;line-height:20px;font-size:12px;padding:3px;background: #fff" type=button  class=wffbtn accessKey=1  onclick="addDocReadTag('<%=showid%>');top.location='/weaver/weaver.file.FileDownload?fileid=<%=docImagefileid%>&download=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>'">
            ...
            <td class="fieldvalueClass" valign="middle" colSpan=3> 
            	<% 
                 ...
                 <%-- 
                 &nbsp;&nbsp;&nbsp;&nbsp;
                 <button style="color:#123885;border:0px;line-height:20px;font-size:12px;padding:3px;background: #fff" type=button class=wffbtn accessKey=1  onclick="top.location='/weaver/weaver.file.FileDownload?fieldvalue=<%=fieldvalue%>&download=1&downloadBatch=1&requestid=<%=requestid%>'">
