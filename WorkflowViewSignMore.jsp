...
//	wfsignlddtcnt=2;
	...
	//System.out.println("sbfmaxrequestlogid2:"+sbfmaxrequestlogid);
}
								...
								<span class="showUsers" style="color:#7e7e7e;<%=isprint ? "display:inline-block;" : "overflow:hidden;float:left;" %>height:17px;margin-bottom: 20px;<%=(log_remarkHtml == null || "".equals(log_remarkHtml.trim())) ? "" : "margin-top:5px;" %>" id="<%=bodydivid%>">
									...
									<%
										...
										if(log_receivedPersons.length()>0) {
											if(log_receivedPersons.trim().endsWith(",")) {
												log_receivedPersons = log_receivedPersons.substring(0,log_receivedPersons.length()-1);
											}
											tempStr =Util.toScreen(log_receivedPersons,languageidfromrequest);
										}
										//if(log_receivedPersons.length()>0){
