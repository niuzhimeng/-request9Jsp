...
//计算权限校验参数
int requestid_authTmp = Util.getIntValue(request.getParameter("requestid"));
String authStr_tmp = "";
String authSignatureStr_tmp = "";
if(requestid_authTmp > -1) {
	RequestAuthenticationService authService = new RequestAuthenticationService();
	authService.setUser(user);
	authService.verify(request, requestid_authTmp);
	authStr_tmp = Util.null2String(authService.getAuthString());
	authSignatureStr_tmp = Util.null2String(authService.getAuthSignatureString());
}
//支持E9表单的直接跳转
boolean haveVerifyForward = "true".equals(Util.null2String(request.getParameter("haveVerifyForward")));
if(!haveVerifyForward){
	/*E9打印登录超时报错(QC:527424)*/
	if(user == null) {
		response.sendRedirect("/wui/index.html");
		return;
	}
	boolean reqSpaPage = ServiceUtil.isReqRoute(Util.null2String(request.getParameter("requestid")),user,Util.getIntValue(request.getParameter("isrequest")));
	if(reqSpaPage){
		String url = "/spa/workflow/index_form.jsp#/main/workflow/req?";
...
if(requestid<0){//异构系统跳转
	...
	if(_workflowid == 0 || _workflowtype == 0){
		rs.executeQuery("select sysid,workflowid from ofs_todo_data where requestid = ?",requestid);
		if(rs.next()){
			_workflowid = rs.getInt("workflowid");
			_workflowtype = rs.getInt("sysid");
		}
	}
	String linkUrl = "/workflow/request/ViewRequestOS.jsp?requestid="+requestid+"&workflowid="+_workflowid+"&sysid="+_workflowtype;
	int f_weaver_belongto_userid = Util.getIntValue(request.getParameter("f_weaver_belongto_userid"),0);
	int f_weaver_belongto_usertype = Util.getIntValue(request.getParameter("f_weaver_belongto_usertype"),-1);
	if(f_weaver_belongto_userid !=0 && f_weaver_belongto_usertype!=-1){
		linkUrl += "&f_weaver_belongto_userid="+f_weaver_belongto_userid;
		linkUrl += "&f_weaver_belongto_usertype="+f_weaver_belongto_usertype;
	}
...
String needclose = (String)session.getAttribute("needclose_"+requestid+"_"+userid);
session.removeAttribute("needclose_"+requestid+"_"+userid);

if("1".equals(needclose)){//解锁流程保存数据后，关闭页面
	%>
...
RecordSet.executeSql("select issignmustinput,isformprint,isformprintLog from workflow_flownode where workflowid="+workflowid+" and nodeid="+nodeid);
if(RecordSet.next()){
...
String delselfLock = Util.null2String(request.getParameter("delselfLock"));
if(needcheckLock){
	LockDTO lockDTO = LockUtil.isLocked(requestid+"",nodeid+"");
	islock = lockDTO.isLock();
	
	if(islock){
		if("1".equals(delselfLock) && (userid+"").equals(lockDTO.getUserid())) //自己又打开了一个新的页面，并且将其他已打开页面置为无效
			islock = false;
	}
	
}
boolean isGotoManagePage= false;//是否原本要跳转到manage页面
if(istoManagePage && !isprint && !wfmonitor && isonlyview!=1 && !"1".equals(isworkflowhtmldoc)){
	if(islock){
		isGotoManagePage = true;
	}else{
	    ...
	    */
			...
			//TD4294  删除workflow_currentoperator表中orderdate、ordertime列 fanggsh begin
		{
			...
			RecordSet.executeSql("update workflow_currentoperator set viewtype=-2,operatedate=( case isremark when '2' then operatedate else date_format(NOW(),'%Y-%m-%d') end  ) ,operatetime=( case isremark when '2' then operatetime else date_format(now(),'%H:%i:%s') end  ) where requestid = " + requestid + "  and userid ="+userid+" and usertype = "+usertype+" and viewtype<>-2 ");
		}
			//TD4294  删除workflow_currentoperator表中orderdate、ordertime列 fanggsh end
	    else
			//TD4294  删除workflow_currentoperator表中orderdate、ordertime列 fanggsh begin
		...
		if("1".equals(delselfLock)){
			session.setAttribute(userid+"_"+requestid+"delselfLock",delselfLock);
		}
	    ...
	    String ehnextnodeid = Util.null2String(request.getParameter("ehnextnodeid"));
	    if(!"".equals(ehnextnodeid))	actionPage+="&ehnextnodeid="+ehnextnodeid;
...
if(isformprint == 1){
	strBar += "{text: '"+printName+"',iconCls:'btn_print',handler: function(){bodyiframe.openSignPrint();}},";
}
...
//TD4262 增加提示信息  结束
	...
	function setbodyiframe(){
		document.getElementById("bodyiframe").src="ViewRequestIframe.jsp?"+bodyiframeurl+'&isGotoManagePage=<%=isGotoManagePage%>';
		//initNewRequestFrame();
