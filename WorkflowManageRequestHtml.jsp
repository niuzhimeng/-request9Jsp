...
function playImgs(src) {
	...
	parent.parent.IMCarousel.showImgScanner4Pool(true, allImgFileArray, indexNum, null, window.top);
}
...
rs_html.executeSql("select isannexUpload , annexdoccategory from workflow_base where id = " + workflowid);
if(rs_html.next()){
...
function openAccessory(fileId){
	...
	jQuery($GetEle("flowbody")).attr("onbeforeunload", "");
	openFullWindowHaveBar("/weaver/weaver.file.FileDownload?fileid="+fileId+"&f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>");
}
...
function changeChildField(obj, fieldid, childfieldid){
    var paraStr = "fieldid="+fieldid+"&childfieldid="+childfieldid+"&isbill=<%=isbill%>&selectedfieldid=<%=selectedfieldid%>&uploadType=<%=uploadType%>&isdetail=0&selectvalue="+obj.value + "&v=<%=Util.getRandom()%>";
    //$G("selectChange").src = "SelectChange.jsp?"+paraStr;
    if($G("iframe_"+fieldid+"_"+childfieldid+"_00")){
        return;
    }
...
function changeChildFieldDetail(obj, fieldid, childfieldid, rownum){
    ...
    if($G("iframe_"+fieldid+"_"+childfieldid+"_"+rownum) && jQuery("#iframe_"+fieldid+"_"+childfieldid+"_"+rownum).html()!=""){
       return;
    }
    var frm = document.createElement("iframe");
	...
	try{
		...
		if(pField != null){
            if(pField.value != "" && jQuery(pField).parent().find(".radioCheckDiv").length > 0 && jQuery(pField).parent().find(".radioCheckDiv").find('input[type="radio"]').length <= 0){
              pField.value = "";
              if(jQuery(pField).attr("viewtype") == "1"){
                  jQuery("#field"+pFieldid+"_"+rownum + "span").html("<img align='absMiddle' src='/images/BacoError_wev8.gif'>");
              }
            }
			var pFieldValue = pField.value;
			if(pFieldValue==null || pFieldValue==""){
...
jQuery(document).ready(function(){
	...
	if(requestimport=="1") {
       ...
       %>	
		if(window.confirm('<%=SystemEnv.getHtmlLabelName(501848, user.getLanguage())%>')) {
