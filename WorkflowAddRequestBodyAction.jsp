...
<%@ page import="weaver.workflow.request.FileUploadDTO"%>
...
while(RecordSet_nf1.next()){
    ...
    isviews.add(Util.getIntValue(RecordSet_nf1.getString("isview"),0) + "");
    isedits.add(Util.getIntValue(RecordSet_nf1.getString("isedit"),0) + "");
    ismands.add(Util.getIntValue(RecordSet_nf1.getString("ismandatory"),0) + "");
}
...
<%
        	...
        	//添加事件信息
        	String uploadMax = "changeforUpload('"+fieldbodyid+"',"+workflowid+","+user.getLanguage()+")";
            ...
            System.out.println("sql============================select childfieldid,type,fieldshowtypes from workflow_formdict where id= " + fieldbodyid);
        	if("0".equals(isbill)){
        		rs_item.executeQuery("select childfieldid,type,fieldshowtypes from workflow_formdict where id= ?" ,fieldbodyid);
        	}else{
        		//rs_item.execute("select childfieldid from workflow_billfield where id="+fieldbodyid);
                rs_item.executeQuery("select childfieldid,type,fieldshowtypes from workflow_billfield where id=?",fieldbodyid);
        	}
        	if(rs_item.next()){
	       		childfieldid_tmp = Util.getIntValue(rs_item.getString("childfieldid"), 0);
	       		
	       		showtype = Util.getIntValue(rs_item.getString("type"), 1);
	       		fieldshowtypes = Util.getIntValue(rs_item.getString("fieldshowtypes"), 1);
	            
        	}
            System.out.println("childfieldid_tmp============================" + childfieldid_tmp);
	   ...
	   <%
                    ...
                    %>
                       ...
                       <% if(showtype == 2){%>
                           <input type='checkbox' <%if(isbodyedit.equals("0")){%>  DISABLED <%}%>name="field<%=fieldbodyid + "_" + tmpselectvalue%>" id="field<%=fieldbodyid + "_" + tmpselectvalue%>"
                             ...
                             <%--
                             onBlur="checkinput2('field<%=fieldbodyid%>','field<%=fieldbodyid%>span',this.viewtype);"
                             onChange="selectItemCheckChange(this,<%=fieldbodyid%>);<%=uploadMax %>;<%if(trrigerfield.indexOf("field"+fieldbodyid)>=0 && !isbodyedit.equals("0")){%>datainput('field<%=fieldbodyid%>');<%}if(selfieldsadd.indexOf(fieldbodyid)>=0){%>changeshowattr('<%=fieldbodyid%>_0',this.value,-1,<%=workflowid%>,<%=nodeid%>);<%}%><%=onchangeAddStr%>;"
                              --%>
                             onClick="selectItemCheckChange(this,<%=fieldbodyid%>);checkinput2('field<%=fieldbodyid%>','field<%=fieldbodyid%>span',this.viewtype);" 
                           >
                       <%} else{%>
                           <input type='radio' <%if(isbodyedit.equals("0")){%>  DISABLED <%}%>name="field<%=fieldbodyid %>_disp" id="field<%=fieldbodyid + "_" + tmpselectvalue%>"
                             value=<%=tmpselectvalue %> 
                             viewtype="<%=isbodymand%>"
                             <%if("".equals(preAdditionalValue)){
                                   if("y".equals(isdefault) && finalvalue.equals("")){ 
                                       finalvalue = tmpselectvalue;
                                       checkempty = false;
                                   %>checked<%}
                               }else{
                             ...
                             <%--
                             onBlur="checkinput2('field<%=fieldbodyid%>','field<%=fieldbodyid%>span',this.viewtype);"
                             onChange="<%=uploadMax %>;<%if(trrigerfield.indexOf("field"+fieldbodyid)>=0 && !isbodyedit.equals("0")){%>datainput('field<%=fieldbodyid%>');<%}if(selfieldsadd.indexOf(fieldbodyid)>=0){%>changeshowattr('<%=fieldbodyid%>_0',this.value,-1,<%=workflowid%>,<%=nodeid%>);<%}%><%=onchangeAddStr%>"
                             --%>
                           >
                       <%} %>
                       <label for="field<%=fieldbodyid + "_" + tmpselectvalue%>" ><%=tmpselectname%></label>
                       </div>
                    <%
				}
            }
      ...
      <%
       ...
       %>
	    ...
	    if(isbodyedit.equals("0") || showtype == 2 || showtype == 3){
	    %>
	    ...
	    <%if(isbodyedit.equals("0")){%>
            ...
            <%if(showtype == 2 || showtype == 3){ %>
             viewtype="<%=isbodymand%>" temptitle="<%=Util.toScreen(fieldbodylable,languagebodyid)%>" 
            <%} %>
            <%if(showtype == 3){ %>
             onpropertychange="<%=uploadMax %>;<%if(trrigerfield.indexOf("field"+fieldbodyid)>=0 && !isbodyedit.equals("0")){%>datainput('field<%=fieldbodyid%>');<%}if(selfieldsadd.indexOf(fieldbodyid)>=0){%>changeshowattr('<%=fieldbodyid%>_0',this.value,-1,<%=workflowid%>,<%=nodeid%>);<%}%><%=onchangeAddStr%>;"            
            <%} %>
         >
      <%}%>
	    <!--xwj for td3313 20051206 end-->
...
<%
       ...
       //add by xhheng @20050310 for 附件上传
    	   ...
    	   FileUploadDTO fudto = FieldFileUploadUtil.getFileUploadInfo(fieldbodyid,workflowid+"","");
			String picfiletypes = fudto.getPicfiletypes();
			String filetypedesc = fudto.getFiletypedesc();
			String mainId = fudto.getMainId();
			String subId = fudto.getSubId();
			String secId = fudto.getSecId();
			uploadType = fudto.getUploadType();
			isCanuse = fudto.isCanuse();
			String temp_SelectedCateLog = fudto.getSelectedCateLog();
			maxUploadImageSize = fudto.getMaxUploadFileSize();
      ...
      %>
          ...
          function fileupload<%=fieldbodyid%>() {
			  ...
			  {
				  settings = {
			selectfieldid : '<%=temp_SelectedCateLog%>',
        ...
        try {
            ...
            mainUploadObjs.push(oUpload<%=fieldbodyid%>);
        } catch(e) {
            alert(e)
        }
    }
      ...
      <TABLE class="ViewForm" style="width:400px;">
          <tr>
              <td colspan="2" style="background-color:#ffffff;">
                     <div>
                      <span id="uploadspan_<%=fieldbodyid %>"  style="display:inline-block;line-height: 32px;"><%=SystemEnv.getHtmlLabelName(83523,user.getLanguage())%><%=maxUploadImageSize%><%=SystemEnv.getHtmlLabelName(83528,user.getLanguage())%>
	                  </span>
	                  <%
						if(isbodymand.equals("1")){
						%>
...
jQuery(function(){
	setTimeout(function(){
		initUploadMax(<%=workflowid%>,<%=user.getLanguage()%>);
	},1000)
})
...
function changeChildField(obj, fieldid, childfieldid){
    ...
    var frm = document.createElement("iframe");
         frm.id = "iframe_"+fieldid+"_"+childfieldid+"_00";
         frm.style.display = "none";
         document.body.appendChild(frm);
         $G("iframe_"+fieldid+"_"+childfieldid+"_00").src = "SelectChange.jsp?"+paraStr;
...
function doInitChildSelect(fieldid,pFieldid,finalvalue){
	try{
		...
		if(pField != null){
		    if(pField.value != "" && jQuery(pField).parent().find(".radioCheckDiv").length > 0 && jQuery(pField).parent().find(".radioCheckDiv").find('input[type="radio"]').length <= 0){
		      pField.value = "";
              if(jQuery(pField).attr("viewtype") == "1"){
                  jQuery("#field"+pFieldid + "span").html("<img align='absMiddle' src='/images/BacoError_wev8.gif'>");
              }
		    }
