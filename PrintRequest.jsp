...
<%@ page import="com.api.workflow.constant.RequestAuthenticationEnum" %>
<%@ page import="com.api.workflow.constant.RequestAuthenticationConstant" %>
...
//打印验证
String authStr = Util.null2String(request.getParameter("authStr"));    //E9 判断流程新参数
String authSignatureStr = Util.null2String(request.getParameter("authSignatureStr"));  //E9 判断流程新参数
RequestAuthenticationService authService = new RequestAuthenticationService();
//rs.writeLog("authStr = "+authStr+"||authSignatureStr = "+authSignatureStr);
if(!canview && (!authStr.isEmpty() && !authSignatureStr.isEmpty())){
	authService.setUser(user);
	authService.setIsprint(true);
	if(authService.verify(request, requestid)){
		...
		if(authEnum.equals(RequestAuthenticationEnum.RELATE) || authEnum.equals(RequestAuthenticationEnum.MAINCHILD)){
			desrequestid = Util.getIntValue(Util.null2String(authInfo.get("desrequestid")),0);
		}
	}
}
...
if(!canview && isrequest.equals("1")){      // 从相关工作流过来,有查看权限
...
if (!canview && (isrequest.equals("2") || isrequest.equals("3") || isrequest.equals("4"))) {
...
int printmodeid=Util.getIntValue(Util.null2String(request.getParameter("modeid")),-1);
int printismode=Util.getIntValue(Util.null2String(request.getParameter("ismode")),-1);
if(printismode==1&&printmodeid>0){
	ismode=""+printismode;
	print_modeid=printmodeid;
	modeid=printmodeid;
}
new weaver.general.BaseBean().writeLog("printRequest.jsp"+modeid);
new weaver.general.BaseBean().writeLog("printRequest.jsp-printismode"+printismode);
new weaver.general.BaseBean().writeLog("printRequest.jsp-printmodeid"+printmodeid);
    ...
    window.setTimeout(function(){		//延时执行，使得图片缩小功能打印自动弹框有效
        try{
            ...
            if(usePrePrint == "true"){
                ...
                try {
                    <%
                        if(!"true".equals(Util.null2String(request.getParameter("isPreView")))){
                            %>
