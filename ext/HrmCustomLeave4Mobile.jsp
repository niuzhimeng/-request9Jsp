...
<%
	...
	int nodeid = StringUtil.parseToInt(request.getParameter("nodeid"), 0);
...
<script language="javascript">
	...
	function initInfo(){
		...
		if(resourceId != '' && fromDate!='' && fromTime!='' && toDate!='' && toTime!=''){
			$.get('/mobile/plugin/1/workflowBillAction.jsp?action=initInfo',{cmd:"leaveInfo", currentDate:fromDate, nodetype:"<%=nodetype%>", workflowid:"<%=workflowid%>",resourceId:resourceId,newLeaveType:newLeaveType, nodeid:"<%=nodeid%>"},
				function process(dataObj){
					if(dataObj) try {
					 	allannualValue=dataObj.allannualValue;
					 	allpsldaysValue=dataObj.allpsldaysValue;
					 	paidLeaveDaysValue=dataObj.paidLeaveDaysValue;
					 	realAllannualValue=dataObj.realAllannualValue;
					 	realAllpsldaysValue=dataObj.realAllpsldaysValue;
					 	realPaidLeaveDaysValue=dataObj.realPaidLeaveDaysValue;
					} catch(e) {
					}
				}
			,"json");
	...
	checkCustomize1 = function(){
		...
		var newLeaveType = jQuery("#"+_field_newLeaveType).val();
		var newLeaveTypeText = jQuery.trim(jQuery("#field"+_field_newLeaveType+"_span").html());
		...
		if(newLeaveType == '<%=HrmAttVacation.L6%>') {
			...
			if(allpsldaysValue <= 0){
		        return "<%=SystemEnv.getHtmlLabelName(382595,user.getLanguage())%>"+newLeaveTypeText+"<%=SystemEnv.getHtmlLabelName(382588,user.getLanguage())%>"+newLeaveTypeText+"<%=SystemEnv.getHtmlLabelName(127389,user.getLanguage())%>";
			}
			if($GetEle(_field_leaveDays)!=null && parseFloat($GetEle(_field_leaveDays).value) > parseFloat(realAllpsldaysValue)){
				return "<%=SystemEnv.getHtmlLabelName(382589,user.getLanguage())%>"+newLeaveTypeText+"<%=SystemEnv.getHtmlLabelName(382590,user.getLanguage())%>"+newLeaveTypeText+"<%=SystemEnv.getHtmlLabelName(382591,user.getLanguage())%>"+"<%=SystemEnv.getHtmlLabelName(127389,user.getLanguage())%>";
			}
		} else if(newLeaveType == '<%=HrmAttVacation.L13%>') {
