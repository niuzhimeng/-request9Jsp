...
<%@page import="weaver.workflow.sysPhrase.WorkflowPhrase"%>
...
<!--
	...
	%>
	var _hasPrivateRight = <%=hasRight%>;
//-->
</script>
<!-- 
<script type="text/javascript" src="/ueditor/custbtn/appwfPhraseCombox_wev8.js"></script>
<script type="text/javascript" src="/ueditor/custbtn/appwfPhraseAddBtn_wev8.js"></script>
 -->
...
try{
    rs.executeSql("select isRemarkLocation from workflow_flownode where workflowid=" + workflowid +"and nodeId=" + nodeid);
	if(rs.next()){
	    isRemarkLocation  = Util.getIntValue(rs.getString("isRemarkLocation"),0); 
	}
}catch(Exception e){
	if(requestid<=0){
	    rs.executeSql("select isRemarkLocation from workflow_flownode where workflowid=" + workflowid +"and nodetype=" + 0);
        if(rs.next()){
            int wfid = Util.getIntValue(rs.getString("workflowid"),0);	            
        }
	}
...
System.out.println("=================================isRemarkLocation=" + isRemarkLocation );
			    		...
			    		<tr>       
			              <td>     
			                <div id="remarkShadowDiv" style="display:inline-block;width:100%;border: 1px solid #cccccc;">
                                	...
                                	<!-- 
                                	<textarea name="remarkShadow" id="remarkShadow" onfocus="initRemark();" style="padding-left:5px;width:85%;border-left-width:0px;resize: none;height:20px;color:#a2a2a2;overflow: hidden;color:#c7c7c7;margin-right:1%;margin: 10px 10px 10px 0;<%if(isSignMustInput.equals("1")){%>border-left:'2px red solid;'<%}%>" temptitle="<%=SystemEnv.getHtmlLabelName(17614, user.getLanguage())%>" ><%=myremark%></textarea>
                                	 -->
                           		</div>
                                ...
                                <div class="signotherblock" style="position: absolute; left: 0; top: 0px;bottom:0px;right:-1px; height:<%=formSignatureHeightInt %>px; background:#f8f8f8;border:1px solid #cccccc;border-top:0px solid #ccc;overflow-y:auto;overflow-x:hidden;">
                                     ...
                                     if (tempbeagenter.equals(userid + "") && !"1".equals(isSignWorkflow_edit) && !"1".equals(isannexupload_edit) && !"1".equals(isannexupload_edit)) {
                                      ...
                                      <div style="margin:5px 10px 5px 10px;font-size:13px;color:#c6c6c6;">
                                       		 <%=SystemEnv.getHtmlLabelName(501849,user.getLanguage())%>
                                      </div>
                                     <%
                                     }
                                     if (!tempbeagenter.equals(userid + "")) {
                                     ...
                                     <div class='signbottommenu' style="left: 0px;width: 100%">
                                        ...
                                        <div id="signAnnexupload" style="">
                                            <%
                                                if (annexsecId < 1) {
                                            %>
                                            <%=SystemEnv.getHtmlLabelName(501847, user.getLanguage())%>!</font>
	                            		</div>             
                                        <%
                                        } else {
                                        %>
