...
if(requestid>0&&!"-1".equals(canviewworkflowid)){
    ...
    for (int k = 0; k < signdocidlist.size(); k++) {
        ...
        if(!isprint&&((!fileExtendName.equalsIgnoreCase("xls")&&!fileExtendName.equalsIgnoreCase("doc"))||!nodownload)){
            returnstr += "<a style=\"cursor:hand\" onclick=\"addDocReadTag('" + docid + "');top.location='/weaver/weaver.file.FileDownload?fileid=" + docImagefileid + "&download=1&requestid=" + requestid + "'\">"
            ...
            if(!"1".equals(forbidAttDownload)&&AttachmentCounts>1 && !nodownload && linknum==0){
              returnstr += "&nbsp;&nbsp;<a style=\"cursor:hand\" onclick=\"addDocReadTag('" + docid + "');top.location='/weaver/weaver.file.FileDownload?fieldvalue=" + docids + "&download=1&downloadBatch=1&requestid=" + requestid + "'\">"
