	...
	<%}else{ 
    	...
    	if ("".equals(aid)){
    	    ...
    	    RecordSet.executeSql("select count(0) agcount from workflow_agent t1,workflow_base t2 " +
                    " where t1.workflowid = t2.id and t1.agenttype='1' and exists( select 1 from workflow_agentConditionSet t3 where t3.agentid=t1.agentId and t3.agentuid = null ) and t1.beagenterId = " + ("".equals(beaid) ? null : beaid)  + swt);
    	}else
    	    RecordSet.executeSql("select count(0) agcount from workflow_agent t1,workflow_base t2 " +
				" where t1.workflowid = t2.id and t1.agenttype='1' and exists( select 1 from workflow_agentConditionSet t3 where t3.agentid=t1.agentId and t3.agentuid='"+aid+"' ) and t1.beagenterId = "+ ("".equals(beaid) ? null : beaid)  + swt);
