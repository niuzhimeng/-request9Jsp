...
if(printmodeid>0){
    ...
    rsz.executeQuery("select 1 from workflow_printset where workflowid=? and nodeid=? and type=4 and modeid=?",workflowid,nodeid,modeid);
    if(rsz.next()){
        isform=1;
    }
}

    new weaver.general.BaseBean().writeLog("printmode.jsp"+modeid);
    new weaver.general.BaseBean().writeLog("printmode.jsp-printmodeid"+printmodeid);
//=========================新增打印日志start=================
...
<jsp:include page="PrintHiddenField.jsp" flush="true">
    ...
    <jsp:param name="printmodeid" value="<%=printmodeid%>" />
