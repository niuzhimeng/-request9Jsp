<%@page import="weaver.fna.maintenance.FnaBatchSubmitControl"%>
...
<%@ page import="weaver.workflow.request.entity.RequestOperateEntityTableNameEnum" %>
...
if(rs.getDBType().equalsIgnoreCase("oracle")){
	...
	dateSql = "select date_format(NOW(),'%Y-%m-%d') as currentdate, date_format(now(),'%H:%i:%s') as currenttime from dual";
}else{
...
int lockCount = 0;
int allCount = requestids.length;
for (int i=0; i<requestids.length; i++){
    ...
    if(RecordSet.getDBType().equals("sqlserver")){
		...
		RecordSet.executeSql("select t1.requestid from workflow_requestbase t1, workflow_currentoperator t2 " +
   		"where t1.requestid = t2.requestid and t2.userid = "+user.getUID()+" and t2.usertype = 0 and (ifnull(t1.currentstatus, -1) = -1 or (ifnull(t1.currentstatus, -1) = 0 and t1.creater = "+user.getUID()+")) "+
   		"and t2.isremark in ('0', '1', '5', '8', '9', '7') and t2.islasttimes = 1 and t1.requestid = " + requestid);
   	    if(!RecordSet.next()){
   	    	continue;
   	    }
...
if (isfrommobile == 1) {
	//手机端约定 result==9的时候，提示信息 取message
	String message = "";
	if("2".equals(result)){
		message = SystemEnv.getHtmlLabelName(128911,user.getLanguage());//部分锁定
		result = "9";
	}else if("3".equals(result)){
		message = SystemEnv.getHtmlLabelName(128912,user.getLanguage());//全部锁定
		result = "9";
	}
    response.getWriter().write("{\"result\":"+result+",\"message\":\""+message+"\"}");
    return;
}
