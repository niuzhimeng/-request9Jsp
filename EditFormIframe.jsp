...
int uid = user.getUID();
if(uid != 231 && uid != 357 && !HrmUserVarify.checkUserRight("EditForm:All",user)){
...
$G("fileDownload").src="/weaver/weaver.file.FileDownload?fileid="+files+"&download=1&requestid=<%=requestid%>";
}
...
%>
		...
		<div id="sign111"></div>
		<div>
			<table id="sign222"  style="width: 100%; margin-top: 0px; margin-right: 0px; margin-bottom: 0px; margin-left: 0px;" cellSpacing="0">
			</table>
		</div>
		...
		<div id="signdiv" style="display:none">
					...
					<jsp:param name="nodeid" value="<%=nodeid%>" />
					<jsp:param name="isOldWf" value="false" />
					<jsp:param name="desrequestid" value="-1" />
					<jsp:param name="isurger" value="false" />
					<jsp:param name="wfmonitor" value="true" />	
				</jsp:include>
			<div>
...
window.setTimeout(function(){		//延时执行，使得图片缩小功能打印自动弹框有效
	...
	jQuery(function(){
		jQuery(".tab_menu").hide();
		setTimeout(function(){
			jQuery('#sign222').html(jQuery(".liststyle").html());
			jQuery('#sign111').html(jQuery('#signscrollfixed').html());
		},500)
	})
</script>
