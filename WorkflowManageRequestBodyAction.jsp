...
<%
	...
	while (RecordSet.next()) {
		...
		isviews.add(Util.getIntValue(RecordSet.getString("isview"),0) + "");
		isedits.add(Util.getIntValue(RecordSet.getString("isedit"),0) + "");
		ismands.add(Util.getIntValue(RecordSet.getString("ismandatory"),0) + "");
	}
      ...
      <%
      				...
      				if (fieldtype.equals("2") || fieldtype.equals("19"))
                       ...
                       for(int k=0;k<tempshowidlist.size();k++){
						...
						}else if (fieldtype.equals("87")|| fieldtype.equals("184")) {//会议室 多会议室
							String[] tmp = fieldvalue.split(",");
							for (int j = 0; j < tmp.length; j++) {
								if (!tmp[j].isEmpty()) {
									String roomname = MeetingRoomComInfo.getMeetingRoomInfoname(tmp[j]);
									if (!linkurl.equals("")) {
						...
						}else if (fieldtype.equals("268")) {// 多星期
...
<%
				...
				if ("0".equals(isbill)) {
					rs_item
							.executeQuery("select childfieldid,type,fieldshowtypes from workflow_formdict where id=?"
									, fieldid);
				} else {
					rs_item
							.executeQuery("select childfieldid,type,fieldshowtypes from workflow_billfield where id=?"
									, fieldid);
				}
				if (rs_item.next()) {
				...
				//添加事件信息
				String uploadMax = "changeforUpload('"+fieldid+"',"+workflowid+","+user.getLanguage()+");";
%>
        <script>
        	function funcField<%=fieldid%>(){
        ...
        <select class=inputstyle notBeauty=true  viewtype="<%=ismand%>" temptitle="<%=Util.toScreen(fieldlable, languageid)%>" onBlur="checkinput2('field<%=fieldid%>','field<%=fieldid%>span',this.viewtype);" <%if (isedit.equals("0") || !editbodyactionflag) {%> name="disfield<%=fieldid%>" DISABLED <%} else {%> name="field<%=fieldid%>" id="field<%=fieldid%>" <%}%> onChange="<%=uploadMax%><%if (trrigerfield.indexOf("field" + fieldid) >= 0 && !(isedit.equals("0") || !editbodyactionflag)) {%>setTimeout(function() {datainput('field<%=fieldid%>')},500);<%}
						if (selfieldsadd.indexOf(fieldid) >= 0) {%>changeshowattr('<%=fieldid%>_0',this.value,-1,<%=workflowid%>,<%=nodeid%>);<%}%><%=onchangeAddStr%>"  >	    <option value=""></option><!--added by xwj for td3297 20051130 -->
	   <%
         }else{
        %>
       ...
       <%
	   	...
	   	// 查询选择框的所有可以选择的值
                ...
                if(showtype == 1){
	   %>
	    <option value="<%=tmpselectvalue%>" <%if (fieldvalue.equals(tmpselectvalue)) {checkempty = false;%> selected <%}%>><%=tmpselectname%></option>
	   <%
                }else{
                    %>
                       ...
                       <% if(showtype == 2){%>
                           <input type='checkbox' <%if(isedit.equals("0") || !editbodyactionflag){%> DISABLED <%}%>name="field<%=fieldid + "_" + tmpselectvalue%>" id="field<%=fieldid + "_" + tmpselectvalue%>"
                             <%-- onBlur="checkinput2('field<%=fieldid%>','field<%=fieldid%>span',this.viewtype);"--%>
                             value=<%=tmpselectvalue %> 
                             <% if(("," + fieldvalue + ",").indexOf("," + tmpselectvalue + ",") >=0){ 
                                 checkempty = false;
                             %>
                              checked 
                              <%} %>
                             viewtype="<%=ismand%>" 
                             onClick="selectItemCheckChange(this,<%=fieldid%>);
                                       checkinput2('field<%=fieldid%>','field<%=fieldid%>span',this.viewtype);" 
                                       ;"
                           >
                       <%} else{%>
                           <input type='radio' <%if(isedit.equals("0") || !editbodyactionflag){%> DISABLED <%}%> name="field<%=fieldid %>_disp" id="field<%=fieldid + "_" + tmpselectvalue%>"
                             <%-- onBlur="checkinput2('field<%=fieldid%>','field<%=fieldid%>span',this.viewtype);"--%>
                             value=<%=tmpselectvalue %> 
                           ...
                           >
                       <%} %>
                       <label for="field<%=fieldid + "_" + tmpselectvalue%>" ><%=tmpselectname%></label>
                       </div>
                    <%
	   	}
	   	}
      ...
      %>
        ...
        <%
        	//if (isedit.equals("0") || !editbodyactionflag) {
            //复选框时，放置隐藏变量联动
            if (isedit.equals("0") || !editbodyactionflag || showtype == 2 || showtype == 3) {
        %>
        
        <input type=hidden class=Inputstyle name="field<%=fieldid%>" id="field<%=fieldid%>" value="<%=showtype == 2?fieldvalue:finalvalue%>" temptitle="<%=Util.toScreen(fieldlable, languageid)%>"
            <%if(showtype == 2 || showtype == 3){ %>
                viewtype="<%=ismand%>" 
                temptitle="<%=Util.toScreen(fieldlable, languageid)%>"
            <%} 
        ...
        %>
            ...
            if(showtype != 2){%>
            onpropertychange="<%=uploadMax %>;
                              <%if (trrigerfield.indexOf("field" + fieldid) >= 0 && !(isedit.equals("0") || !editbodyactionflag)) {%>datainput('field<%=fieldid%>');<%}%>
...
%>
          ...
          <tr>
            <td class="fieldvalueClass" colspan=3>
             <%
								FileUploadDTO fudto = FieldFileUploadUtil.getFileUploadInfo(fieldid,workflowid+"",requestid+"");
        ...
        try {
            ...
            mainUploadObjs.push(oUpload<%=fieldid%>);
        } catch(e) {
            alert(e)
        }
    }
      ...
      <TABLE class="ViewForm">
          <tr>
              <td colspan="2" style="background-color:#ffffff;">
                  ...
                  <span id="uploadspan_<%=fieldid %>" style="display:inline-block;line-height: 32px;"><%=SystemEnv.getHtmlLabelName(83523,user.getLanguage())%><%=maxUploadImageSize%><%=SystemEnv.getHtmlLabelName(83528,user.getLanguage())%></span>
				  <%
				  if(ismand.equals("1") && fieldvalue.equals("")) {
				  %>
