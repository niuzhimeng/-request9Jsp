...
%>
        ...
        }catch(e){if(window.console)console.log(e);} //end
<%      }// end for
        }catch(Exception e){
            System.out.println("生成清理的js出现异常："+e.getMessage());
        }
        try{
                ...
                if(DDI.GetIsCycle().equals("1")){
                    for(int i=0;i<outdatasList.size();i++){
                        ...
                        for(int j=0; j<outfieldnamelist.size(); j++){
                            ...
                            String js=DDI.ChangeMainField(tempName,tempValue,isbill,nodeid,triggerfieldname);
...
try{
    ...
    eval(mainjs);
    if(window.console) console.log('#field12069==========================================' + parent.jQuery('#field12069').val());
...
public int getNewGroupid(int billid,int groupid){
	...
	if(rs.getDBType().equals("oracle")){
	    ...
	    sql = " SELECT t.rid FROM (select count(*) as rid,orderid from (SELECT tablename, orderid FROM Workflow_billdetailtable  WHERE billid = "+billid+" order by id) t1) t WHERE t.orderid="+groupid ;
	}else{
