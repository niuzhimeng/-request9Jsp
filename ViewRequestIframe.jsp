...
takisremark=Util.getIntValue(Util.null2String((String)session.getAttribute(userid+"_"+requestid+"takisremark")),0);
...
int toexcel=0;
int isformprint = 0;
int isformprintLog = 0;
RecordSet.executeSql("select ismode,showdes,printdes,toexcel,isformprint,isformprintLog from workflow_flownode where workflowid="+workflowid+" and nodeid="+nodeid);
if(RecordSet.next()){
	...
	isformprint = Util.getIntValue(RecordSet.getString("isformprint"), 0);
	isformprintLog = Util.getIntValue(RecordSet.getString("isformprintLog"), 0);
}
...
boolean needcheckLock = LockUtil.isNeedCheckLock(requestid,workflowid,nodeid,Util.getIntValue(isremarkForRM,0),takisremark,userid);
boolean islock = false;
LockDTO lockDTO = null;

if(needcheckLock){
	lockDTO = LockUtil.isLocked(requestid+"",nodeid+"");
	islock = lockDTO.isLock();
}
String isGotoManagePage = Util.null2String(request.getParameter("isGotoManagePage"));//是否原本要跳转至manage页面
%>
...
%>
    ...
    //意见分栏显示，增加分隔线
    try{
	    jQuery("table[id^='formremark_']").each(function(){
			var color = jQuery(this).closest("td").css("border-top-color");//上边框
			if(color == 'rgb(0, 0, 0)'){
				color = jQuery(this).closest("td").css("border-bottom-color");//下边框
				if(color == 'rgb(0, 0, 0)'){
					color = jQuery(this).closest("td").css("border-left-color");//左边框
					if(color == 'rgb(0, 0, 0)'){
						color = jQuery(this).closest("td").css("border-left-color");//右边框
					}
				}
			}
			var i = 0;
			jQuery(this).find("tr:eq(0) td").each(function(){
				if(jQuery(this).attr("needline") == 'true'){
					if(i > 0){
						jQuery(this).css("border-left","solid 1px " + color);
					}
					i++;
				}
			}) 
...
function dealSelfLock(){
	top.window.location.href = "/workflow/request/ViewRequest.jsp?requestid=<%=requestid%>&delselfLock=1";
}
...
if(RecordSet.next()){
	if(user.getLanguage() == 7){
		...
		printName = Util.null2String(RecordSet.getString("printName7"));
		printLogName = Util.null2String(RecordSet.getString("printLogName7"));
	}
		...
		printName = Util.null2String(RecordSet.getString("printName9"));
		printLogName = Util.null2String(RecordSet.getString("printLogName9"));
	}
	else{
		...
		printName = Util.null2String(RecordSet.getString("printName8"));
		printLogName = Util.null2String(RecordSet.getString("printLogName8"));
	}
...
if(RecordSet.next()) {
	...
	if ("".equals(printName)) {
		printName = Util.processBody(Util.null2String(RecordSet.getString("printName")).trim(), "" + user.getLanguage());
	}
	if ("".equals(printLogName)) {
		printLogName = Util.processBody(Util.null2String(RecordSet.getString("printLogName")).trim(), "" + user.getLanguage());
	}
}
if ("".equals(printName)) {
	printName = SystemEnv.getHtmlLabelName(257, user.getLanguage());
}
if ("".equals(printLogName)) {
	printLogName = SystemEnv.getHtmlLabelName(21533, user.getLanguage());
}
...
if(isformprint == 1){
	RCMenu += "{"+printName+",javascript:openSignPrint(),_self}" ;
	RCMenuHeight += RCMenuHeightStep ;
	strBar += "{text: '"+printName+"',iconCls:'btn_print',handler: function(){openSignPrint();}},";
}

if(isformprintLog==1){
RCMenu += "{"+printLogName+",javascript:doPrintViewLog(),_self}" ;
RCMenuHeight += RCMenuHeightStep ;
strBar += "{text: '"+printLogName+"',iconCls:'btn_collect',handler: function(){doPrintViewLog();}},";
}
...
if("true".equals(isGotoManagePage)){
...
enableAllmenu();
 ...
 <%
			    	if(StringUtil.isNotNull(message) || (islock && "true".equals(isGotoManagePage))){
			    		String iconCss = "";
			    		String messageContent = "";
			    		if(StringUtil.isNotNull(message)){
			    			iconCss = "message-title-icon";
			    			messageContent = WorkflowRequestMessage.getNewMessageId(message,user.getLanguage());
			    		}else{
			    			iconCss = "lock-icon";
			    			String lockuserid = lockDTO.getUserid();
			    			if(!(userid + "").equals(lockuserid)){
			    				if(user.getLanguage() == 7){
			    					messageContent = "当前流程已被 <a style='color:blue;' onclick=\"pointerXY(event);\" href=\"javaScript:openhrm("+lockuserid+");\">"+ResourceComInfo.getResourcename(lockuserid)+"</a> 于" + lockDTO.getLockdate() + " " + lockDTO.getLocktime() + " 签出，您暂时不能对表单数据进行操作";
			    				}else if(user.getLanguage() == 9){
			    					messageContent = "當前流程已被 <a style='color:blue;' onclick=\"pointerXY(event);\" href=\"javaScript:openhrm("+lockuserid+");\">"+ResourceComInfo.getResourcename(lockuserid)+"</a> 於" + lockDTO.getLockdate() + " " + lockDTO.getLocktime() + " 簽出，您暫時不能對表單數據進行操作";
			    				}else{
			    					messageContent = "The request has been checked by <a style='color:blue;' onclick=\"pointerXY(event);\" href=\"javaScript:openhrm("+lockuserid+");\">"+ResourceComInfo.getResourcename(lockuserid)+"</a> at " + lockDTO.getLockdate() + " " + lockDTO.getLocktime() + ",you can not operate now";
			    				}
			    			}else{
			    				if(user.getLanguage() == 7){
			    					messageContent = "当前流程已被您于" + lockDTO.getLockdate() + " " + lockDTO.getLocktime() + "  签出，请在已打开的窗口继续操作，或<a notreplace='true' style='color:blue;' href=\"#\" onclick=\"javascript:dealSelfLock();\" target=\"_self\">点击这里在当前窗口处理流程</a>（原打开窗口将失效）";
			    				}else if(user.getLanguage() == 9){
			    					messageContent = "當前流程已被您於" + lockDTO.getLockdate() + " " + lockDTO.getLocktime() + "  簽出，請在已打開的視窗繼續操作，或<a style='color:blue;' href=\"#\" onclick=\"javascript:dealSelfLock();\" target=\"_self\">點擊這裡在當前視窗處理流程</a>（原打開視窗將失效）";
			    				}else{
			    					messageContent = "The request has been checked by yourself at " + lockDTO.getLockdate() + " " + lockDTO.getLocktime() + ",you can operate now or you can <a style='color:blue;' href=\"#\" onclick=\"javascript:dealSelfLock();\" target=\"_self\">click here</a> to unlock the request(the early window will be invalid)";
			    				}
			    			}
			    		}
			    	%>
...
function downloads(files) { 
	...
	$GetEle("fileDownload").src="/weaver/weaver.file.FileDownload?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&fileid="+files+"&download=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>";
}
...
function openDocExt(showid,versionid,docImagefileid,isedit){
	...
	if(isedit==1){
		...
		openFullWindowHaveBar("/docs/docs/DocDspExt.jsp?id="+showid+"&imagefileId="+docImagefileid+"&isFromAccessory=true&isrequest=1&f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>");
	}
}

function openAccessory(fileId){ 
	jQuery($GetEle("flowbody")).attr("onbeforeunload", "");
	openFullWindowHaveBar("/weaver/weaver.file.FileDownload?fileid="+fileId+"&f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>");
}
...
jQuery(document).ready(function(){
	jQuery("span a:contains('<%=SystemEnv.getHtmlLabelName(126555,user.getLanguage()) %>')").each(function(){
		if(jQuery(this).html() != '点击这里在当前窗口处理流程')
			jQuery(this).replaceWith('<%=SystemEnv.getHtmlLabelName(126555,user.getLanguage()) %>')
	});
