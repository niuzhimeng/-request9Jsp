...
if(!prjids.equals("")&&!prjids.equals("0")){
	...
	if(RecordSet.getDBType().equals("oracle")){
		...
		sqlwhere += " and (concat(',' , workflow_requestbase.prjids , ',') LIKE '%,"+prjids+",%') ";
	}else{
...
if(!crmids.equals("")&&!crmids.equals("0")){
	...
	if(RecordSet.getDBType().equals("oracle")){
		...
		sqlwhere += " and (concat(',' , workflow_requestbase.crmids , ',') LIKE '%,"+crmids+",%') ";
	}else{
...
if(RecordSet.getDBType().equals("oracle"))
{
	...
	sqlwhere += " and (ifnull(workflow_requestbase.currentstatus,-1) = -1 or (ifnull(workflow_requestbase.currentstatus,-1)=0 and workflow_requestbase.creater="+user.getUID()+")) ";
}
else
{
...
if(RecordSet.getDBType().equals("oracle")){
	...
	sqlstr = "(select distinct workflow_requestbase.requestid ,requestname,creater,creatertype,createdate,createtime from workflow_requestbase , workflow_currentoperator , workflow_base" + sqlwhere + " and workflow_currentoperator.requestid = workflow_requestbase.requestid  and workflow_currentoperator.usertype="+usertype+" and workflow_requestbase.workflowid = workflow_base.id and (workflow_base.isvalid='1' or workflow_base.isvalid='3') order by createdate desc , createtime desc limit "+(pagenum*perpage+1)+" ) as s" ;
}else{
...
if(RecordSet.getDBType().equals("oracle")){
	...
	sqltemp="select * from "+sqlstr+"  order by createdate , createtime limit "+(RecordSetCounts-(pagenum-1)*perpage)+" ";
}else{
