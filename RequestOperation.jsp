...
System.out.println("submitTokenKey:"+submitTokenKey+",submitToKenValue:"+submitToKenValue+",catchTokenValue:"+catchTokenValue);
...
RequestManager.setDocView(docView);
...
if(docFlag) 
{
 ...
 havaChage=RequestDoc.haveChage(""+workflowid,""+requestid,fu.getRequest());  //如果改变了发文目录
...
if(method.equals("")){  
	...
	if (docFlag){
		...
		RequestDoc.changeDocFiled(""+workflowid,""+requestid,fu.getRequest(),user.getLanguage(),havaChage); //如果改变了发文目录
...
//add by fanggsh 2007-03-23  for TD6247  调整为：在流程中以TAB页形式打开相关文档时才保存流程与模板对应数据  开始
		...
		if (docFlag) RequestDoc.changeDocFiled(""+workflowid,""+requestid,fu.getRequest(),user.getLanguage(),havaChage);  //如果改变了发文目录
