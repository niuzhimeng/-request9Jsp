...
<%
    ...
    if(loadtree.equals("true")){
        ...
        if(requestutil.getOfsSetting().getIsuse() == 1){
            ...
            //查询流程分类数据
            int wftype_count = 0 ;
            rs.executeSql("select sysid,sysshortname from ofs_sysinfo where cancel=0 order by sysid desc ");
            ...
            while(rs.next()){
                ...
                while(RecordSet.next()){
                    ...
                    rs1.executeSql("select count(*) from ofs_todo_data where creatorid="+user.getUID()+" and creatorid=userid  and islasttimes=1 and isremark='0' and workflowid="+_wfid+" and sysid="+_typeid);
                    int _wfnewcount = 0;
                    if(rs1.next()){
...
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
		...
		<script type="text/javascript">
			...
			{
				var ajax=ajaxinit();
       			ajax.open("POST", "/workflow/request/MyRequestView.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&offical=<%=offical%>&officalType=<%=officalType%>", true);
