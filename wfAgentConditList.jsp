            		...
            		if(selectBeginDate.equals("1"))
          			...
          			if(agentstatus.equals("1")){
          			  ...
          			  if(RecordSet.getDBType().equals("oracle")){//标准开发时，未针对oracle数据库做特使控制导致oracle查询不到
						  ...
						  sqlCondition += swt + "  and (date_format(concat(ifnull(beginDate,'1900-01-01') , ' ' , ifnull(beginTime,'00:00')),'%Y-%m-%d %H:%i:%s') <= date_format('"+currentDate+" "+currentTime+"','%Y-%m-%d %H:%i:%s'))";
						}else{
