...
function doSubmitBack(obj){
	...
	}else if(IsFreeWorkflow == "true" && freewftype == "1"){
		document.getElementById("needwfback").value = "1";
		getRemarkText_log();
		doFreeWorkflow();
	} else{
...
function showPrompt(content)
{
    ...
    function doFreeWorkflow() {
		...
		　　dlg.URL = "/workflow/request/FreeNodeShow.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&workflowid=<%=workflowid%>";
			...
			dlg.Width = 850;//定义长度
			dlg.Height = 600;
			dlg.URL = "/workflow/request/FreeNodeWin.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&isdialog=1&isset=1&tt=<%=new Date().getTime()%>";
			<%}%>
